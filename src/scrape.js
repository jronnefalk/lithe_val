...
async function scrape(addresses) {
    ...
    for (let i = 0; i < addresses.length; i++) {
        ...
        const huvudomrade = await page.evaluate(() => {
            const h2 = Array.from(document.querySelectorAll('.overview-label')).find(el => el.textContent.includes('Huvudområde'));
            return h2.nextSibling.textContent.trim().split(' ');
        });

        const utbildningsniva = await page.evaluate(() => {
            const h2 = Array.from(document.querySelectorAll('.overview-label')).find(el => el.textContent.includes('Utbildningsnivå'));
